...
| [graph-storage.md](graph-storage.md) | Where graphs live: canonical `<repo>/.cortex/db`, the central project registry (`_registry.db`), the legacy cache + migration, and the read/write path resolution. Anything under `src/db/` or the index write/read paths. |
