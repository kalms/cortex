...
**Trigger:** User noticed anthill-cloud had no frames in the viewer. Root-causing that led to a quality assessment of the frames it *does* produce, across two repos.

---

## TL;DR

Frame extraction's **clustering is good** — it finds real architectural seams on both a Nuxt monorepo (anthill-cloud) and Cortex's own polyglot tree. The **labeling is the weak link**, and it fails in two specific, reproducible ways:

1. **Label-with-no-domain-token** — when a cluster forms on a *structural* directory that legitimately spans several domains, there is no shared topical token, and the picker grabs an arbitrary leaf. Worked example: anthill's `activator email` (7 sibling feature pages: banners, briefs, email, slides, presentations… labeled after one of them).
...
A first pass blamed anthill's *layout*. That was wrong (see "Correction" below). anthill-cloud is well organized; the labeler is defeated by **framework idioms** (route params `[orgId]`, convention prefixes `use*Store`), not by poor naming. Because anthill is well organized, its awkward labels are unambiguously the **tool's** fault — which makes it the sharper test case, not the weaker one.
...
|---|---|---:|---:|
| anthill-cloud | 152 | 9 | Nuxt 3 monorepo (`apps/* + packages/*`) |
...
**This is not an anthill naming problem.** The cluster boundary fell on a structural directory that genuinely contains multiple domains; the labeler has no fallback for that case.
...
- anthill: `arcane drizzle` (13) — mostly `drizzle/meta/000N_snapshot.json` migration artifacts; `drizzle config` (8) — a junk-drawer of `*.config.ts` + `seed.ts` + scripts + a test.
- cortex: `indexer tools` (11) — generated tree-sitter grammars (`parser.c`, `grammar.js`, `node-types.json`).

The auxiliary-detection pass correctly peeled off some of this (anthill's `css`, `momentum-728x90`, `studio-assets` grouped as auxiliary), but generated migrations, configs, and vendored grammars slipped through into "real" frames.
...
The raw C binary `cortex-indexer cli index_repository` builds the graph but never runs it. anthill-cloud (and cortex) had been reindexed via the raw C binary during unrelated verification work, leaving frameless graphs with **no warning**. Re-running through `cortex index` restored frames (anthill 152/9, cortex 118/8). This is an easy footgun for any tool, hook, or script that drives the C binary directly.
...
Use these two repos as a fixed regression pair for frame quality: anthill-cloud (feature×layer monorepo, framework-idiom-heavy) and cortex (subsystem-partitioned polyglot). They exercise opposite failure modes. A good change should improve anthill's labels **without** regressing cortex's already-clean ones, and should split cortex's `cli commands` blob.

---

## Appendix — frames as evaluated

**anthill-cloud (9):** `orgid design` (35, design-system API routes) · `dsl compiler` (30, ✓) · `arcane server` (23, cross-app server utils) · `activator design` (14, design-system pages) · `arcane drizzle` (13, migrations) · `use store` (12, Pinia stores) · `account` (10, admin pages, mixed) · `drizzle config` (8, junk-drawer) · `activator email` (7, **Worked example A**)
