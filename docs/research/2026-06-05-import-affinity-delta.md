...
|---|---|---|---|---|---|
| anthill-cloud | 10→10 | 53%→51% | 6 | 1 | **0 / 5** |
...
1. **It churns membership without achieving the goal.** Files move both directions, but the net coverage is flat-to-negative — cortex and commerce get *worse* (66→67%, 68→72%); only anthill improves (−2pt), and that −2pt is *other* files. **0 of 8 named targeted files** were rescued at any safe delta.

2. **Root cause — edge sparsity vs over-merge, with no safe global value.** The targeted framework leaves have too few, too-diffuse edges to pull on (anthill's DS leaves: **1–3** cross-file import/call edges each, pointing at shared utils rather than the design-system cluster core; commerce routes: 4–14, but already clustered). A `delta` low enough to be safe on a dense monorepo (cortex) is too weak to move those sparse-edge files; a `delta` high enough to move them collapses the dense graph. **There is no single corpus-wide `delta` that helps sparse-edge framework files without over-merging dense ones.**
