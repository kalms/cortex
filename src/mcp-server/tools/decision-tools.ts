...
export function registerDecisionTools(
  ...
  // ---------------------------------------------------------------------------
  // Per-call repo-scoped service construction has moved to the exported
  // *Action functions above (serviceForCtx / searchForCtx). Each registration
  // below now delegates to the matching extracted function — ONE source of
  // behavior, shared with the `decision` dispatcher (decision-dispatcher.ts).
  // `bus` / `indexerProject` (server-scoped, not repo-scoped) flow through as
  // params, exactly as the former closures captured them.
  //
  // NOTE on `why_was_this_built`: it is the ONLY freshness-aware decision
  // action. Freshness is now applied INSIDE whyWasThisBuiltAction (via
  // attachWhyFreshness), so this registration uses `{ resolver }` WITHOUT
  // `freshnessAware: true` — otherwise freshness would attach twice. get/search
  // and the rest were never freshness-aware and stay that way. See the
  // freshness-parity note in decision-dispatcher.ts.
  // ---------------------------------------------------------------------------
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => createDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => proposeDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => supersedeDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => updateDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => deleteDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      "get_decision",
      getDecisionSchema,
      (ctx, args) => getDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => searchDecisionsAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      // Freshness is applied INSIDE whyWasThisBuiltAction — drop the wrapper's
      // `freshnessAware: true` so it isn't applied twice.
      (ctx, args) => whyWasThisBuiltAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      "link_decision",
      linkDecisionSchema,
      (ctx, args) => linkDecisionAction(ctx, args, bus, indexerProject),
      { resolver },
    ),
  );
  ...
  server.tool(
    ...
    registerTool(
      ...
      (ctx, args) => decisionCandidatesAction(ctx, args),
      { resolver },
    ),
  );
}
